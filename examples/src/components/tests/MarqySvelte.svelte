...
<div class="@container">
  <div class="flex flex-col items-center justify-between gap-20 bg-white/10 p-20 rounded-5 mb-10 @min-[60rem]:flex-row">
    <label class="flex items-center gap-x-10 w-full @min-[60rem]:w-auto">
      ...
      <div class="flex p-3 bg-white/10 rounded-3 grow">
        <input type="range" min="0.1" max="2" step="0.1" bind:value={speed} class="bg-white/30 h-14 min-w-150 rounded-2" />
      </div>
      <span class="text-p0 shrink-0 tabular-nums text-neon">{speed.toFixed(1)}</span>
    </label>
    <div class="flex justify-between w-full border-t border-white/20 pt-20 @min-[60rem]:pt-0 @min-[60rem]:border-0 @min-[60rem]:contents">
      <label class="flex items-center gap-x-10">
        <span class="text-p0">direction</span>
        <select bind:value={direction} class="text-neon bg-neon/20 rounded-3 uppercase h-20 tracking-10 text-p0 outline-none appearance-none px-5 w-auto text-center">
          <option>left</option>
          <option>right</option>
          <option>up</option>
          <option>down</option>
